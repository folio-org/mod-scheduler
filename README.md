...
| KAFKA_TENANT_FILTER_ALL_TENANTS_DISABLED_STRATEGY  | fail                                                                 | Strategy when no tenants at all are entitled (e.g. during startup). `skip` discards the record; `fail` retries with backoff (per the configured retry attempts) and discards the record once retries are exhausted.                                                                                                                                                                                                                                  |
| KAFKA_TENANT_FILTER_RECONCILIATION_INTERVAL        | 5m                                                                   | How often the entitled-tenants cache is fully re-fetched from the entitlement service, correcting any drift from a missed or duplicate entitlement event.                                                                                                                                                                                                                                                                                            |
...
When `KAFKA_TENANT_FILTER_ENABLED=true`, only `scheduled-job` events for tenants currently
