...
| FOLIO_KAFKA_TENANT_FILTER_ALL_TENANTS_DISABLED_STRATEGY | fail                                                                 | Strategy when no tenants at all are entitled (e.g. during startup). `skip` discards the record; `fail` retries with backoff until tenants become entitled.                                                     |
