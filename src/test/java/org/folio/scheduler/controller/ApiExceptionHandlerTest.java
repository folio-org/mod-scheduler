...
import org.folio.scheduler.controller.ApiExceptionHandlerTest.TestController;
...
class ApiExceptionHandlerTest {
  ...
  void handleRequestValidationException_positive() throws Exception {
    ...
    mockMvc.perform(get("/tests").queryParam("query", "cql.allRecords=1").contentType(APPLICATION_JSON))
      ...
      .andExpect(jsonPath("$.errors[0].parameters[0].key").doesNotExist())
      .andExpect(jsonPath("$.errors[0].parameters[0].value").doesNotExist());
  }
  ...
  @Test
  void handleCronValidationException_positive() throws Exception {
    when(testService.getTestValue()).thenThrow(new RuntimeException("CronExpression '*/5 **' is invalid."));
    mockMvc.perform(get("/tests")
        .queryParam("query", "cql.allRecords=1")
        .contentType(APPLICATION_JSON))
      .andExpect(status().isBadRequest())
  ...
  @Test
  void handleTimerSchedulingException_positive() throws Exception {
